...
from .contribution import generate_contribution_recommendations
...
def build_parser() -> argparse.ArgumentParser:
    ...
    contribute = subparsers.add_parser("contribute", help="rank open issues in repositories you already follow")
...
def print_contributions(recommendations: list[IssueRecommendation], warning: str | None = None) -> None:
    """
...
def run_contribute(storage: Storage, limit: int, unassigned_only: bool = False) -> int:
    """
    rank and display open issues in repositories the user already follows
    :param storage: local storage manager
    ...
    if not starred and not interested:
        print("No relevant repositories are available yet.")
        print("Save a recommendation or run `python -m repo_radar sync` to bring in your GitHub stars.")
        return 0
    imported = storage.load_imported_profile()
    ...
    print_contributions(recommendations, warning)
    return 0
...
def main(arguments: list[str] | None = None) -> int:
    ...
    try:
        ...
        if parsed.command == "contribute":
            return run_contribute(storage, parsed.limit, parsed.unassigned_only)
